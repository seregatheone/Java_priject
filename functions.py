...
def first_join(user_id, name):
    ...
    if len(row) == 0:
        cursor.execute(
            f'INSERT INTO users VALUES ("{user_id}", "{name}", "{datetime.datetime.now()}", "0", "0", "0", "0")')
        conn.commit()
        ...
        cursor.execute(
            f'INSERT INTO users VALUES ("{user_id}", "{name}", "{datetime.datetime.now()}", "{settings.LOGIN_ADMIN}", "{settings.LOGIN_BOT}")')
        conn.commit()
        ...
        for i in range(len(req['data'])):
            if comment in str(req['data'][i]['comment']):
                if float(req["data"][i]["sum"]["amount"]) >= 7.0:
                    ...
                    cursor.execute(
                        f'INSERT INTO log VALUES ("{settings.LOGIN_BOT}", "{settings.LOGIN_ADMIN}", "{sum_p}", "{sum_p * settings.PERCENT_SPAM}", "{sum_p * settings.PERCENT_OWN}", "{user_id}", "{datetime.datetime.now()}")')
                    conn.commit()
...
def edit_price(dict):
    try:
        ...
        cursor.execute(
            f'UPDATE "{catalog[int(dict.catalog)][1]}" SET price = "{dict.sum}" WHERE code = "{row[int(dict.product)][2]}"')
        conn.commit()
